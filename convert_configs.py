...
def main():
    parser = argparse.ArgumentParser(
        description="Convert configuration files between YAML and JSON formats"
    )
    parser.add_argument(
        "directories",
        nargs="+",
        help="Directories containing configuration files to convert",
    )
    parser.add_argument(
        "--format",
        choices=["yaml", "json"],
            ...
            if args.backup:
                ...
                for file in files:
                    if os.path.isfile(file):
                        shutil.copy2(
                            file, os.path.join(backup_dir, os.path.basename(file))
                        )

            # Perform the conversion
            converted_files = convert_configs_in_directory(
                directory, target_format=args.format, recursive=args.recursive
            )
        except Exception:
            pass

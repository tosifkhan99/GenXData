...
def excelWriter(df, params):
    ...
    try:
        ...
        path = params.get('output_path') or params.get('path_or_buf') or params.get('excel_writer')
        if not path:
            raise ValueError("Missing path parameter (use 'output_path', 'path_or_buf', or 'excel_writer') for Excel writer")
        ...
        excel_params = {k: v for k, v in params.items() if k not in ['output_path', 'path_or_buf', 'excel_writer']}

...
class PerformanceTracker:
    ...
    def report(self):
        ...
        lines.append("-" * 110)
        lines.append(f"{'Operation':<30} {'Count':<10} {'Total (s)':<12} {'Avg (s)':<12} {'Min (s)':<12} {'Max (s)':<12} {'Rows/sec':<12}")
        lines.append("-" * 110)
        
        for operation, data in sorted(self.metrics.items(), key=lambda x: x[1]['total_time'], reverse=True):
            avg_time = data['total_time'] / data['count'] if data['count'] > 0 else 0
            rows_per_sec = data['rows_processed'] / data['total_time'] if data['total_time'] > 0 and data['rows_processed'] > 0 else None
            ...
            lines.append(f"{operation:<50} {data['count']:<10} {data['total_time']:<12.4f} {avg_time:<12.4f} {data['min_time']:<12.4f} {data['max_time']:<12.4f} {rows_per_sec_str:<12}")

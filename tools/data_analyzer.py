...
class DataAnalyzer:
    ...
    def analyze_numeric_columns(self) -> Dict[str, Dict[str, Any]]:
        ...
        for col in numeric_cols:
            ...
            try:
                ...
                analysis["normality_test"] = {"error": "Could not perform normality test"}
    ...
    def generate_distribution_plots(self) -> None:
        ...
        if len(numeric_cols) > 0:
            ...
            plt.tight_layout()
            plt.savefig(self.output_dir / "plots" / "numeric_distributions.png", dpi=300, bbox_inches='tight')
            plt.close()
        ...
        if len(categorical_cols) > 0:
            for col in categorical_cols[:6]:  # Limit to first 6 categorical columns
                ...
                plt.tight_layout()
                plt.savefig(self.output_dir / "plots" / f"categorical_{col.replace(' ', '_')}.png", 
    ...
    def generate_data_quality_report(self) -> Dict[str, Any]:
        ...
        quality_report["completeness"] = {
            ...
            "columns_with_issues": missing_percentage[missing_percentage > 10].to_dict()
        }
        ...
        for col in numeric_cols:
            if col in self.analysis_results.get("numeric_analysis", {}):
                outlier_percentage = self.analysis_results["numeric_analysis"][col].get("outliers", {}).get("percentage", 0)
        ...
        quality_report["overall_score"] = round((completeness_score + consistency_score) / 2, 2)
...
"""
        ...
        if categorical_analysis:
            ...
            for col, stats in categorical_analysis.items():
                if "error" not in stats:
                    summary += f"• {col}:\n"
                    summary += f"  - Unique Values: {stats.get('unique_count', 'N/A')}\n"
                    summary += f"  - Most Frequent: {stats.get('most_frequent', 'N/A')}\n"
                    summary += f"  - Missing: {stats.get('missing_count', 'N/A')}\n\n"
    ...
    def run_full_analysis(self, format_type: str = "json") -> None:
        ...
        print(f"📊 Summary: {self.analysis_results['basic_info']['total_rows']} rows, "
              f"{self.analysis_results['basic_info']['total_columns']} columns")
        print(f"🔍 Data Quality Score: {self.analysis_results['data_quality']['overall_score']}%")

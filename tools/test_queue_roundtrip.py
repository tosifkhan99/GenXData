...
from queue.amqp_consumer import AMQPConsumer
...
def test_message_handler(expected_test_id):
    ...
    def handler(message_data, raw_message):
        ...
        if message_data.get('test_id') == expected_test_id:
            ...
            print(f"⏰ Roundtrip time: {time.time() - message_data['timestamp']:.3f} seconds")
...
Examples:
  ...
  python tools/test_queue_roundtrip.py --timeout 30
    ...
    try:
        ...
        try:
            producer.connect()
            print(f"✅ Connected to queue as producer")
        ...
        try:
            print(f"🔗 Connecting as consumer...")
            consumer.connect()
            print(f"✅ Connected to queue as consumer")
        ...
        if found_message:
            ...
            print(f"✅ Message was successfully sent and received")
            print(f"✅ Queue messaging pipeline is working correctly")
            return 0
        else:
            print(f"\n❌ FAILURE: Test message not found")
            print(f"⚠️  Possible issues:")
            print(f"   • Message not delivered to queue")
            print(f"   • Queue connection problems")
            print(f"   • Message consumed by another consumer")

...
class ErrorHandler:
    ...
    def _generate_debug_info(self, error: Union[GenXDataError, Exception]):
        ...
        if isinstance(error, GenXDataError) and error.is_critical():
            try:
                ...
                self.logger.critical(f"Critical error debug report exported to: {report_path}")
            except Exception as e:
                self.logger.error(f"Failed to export debug report: {str(e)}")

    def _log_error_by_severity(self, error: Union[GenXDataError, Exception], severity: ErrorSeverity):
    ...
    def generate_error_report(self):
        ...
        self.logger.error(f"=== ERROR REPORT ===")
        ...
        if critical_errors:
            ...
            for error_info in critical_errors:
                self.logger.critical(f"[{error_info['error_code']}] {error_info['message']}")
                if error_info.get('file') and error_info.get('line'):
                    self.logger.critical(f"  Location: {error_info['file']}:{error_info['line']}")
        ...
        for severity in [ErrorSeverity.ERROR, ErrorSeverity.WARNING, ErrorSeverity.INFO]:
            ...
            if errors:
                ...
                for error_info in errors:
                    self.logger.error(f"[{error_info['error_code']}] {error_info['message']}")
    ...
    def export_detailed_error_report(self):
        ...
        for error_info in self.get_errors():
            serializable_error = {
                ...
                'severity': error_info.get('severity').value if error_info.get('severity') else None,
                'category': error_info.get('category').value if error_info.get('category') else None,
        ...
        error_report_path = f"./output/error_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        try:

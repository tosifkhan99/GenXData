...
class DebugFormatter:
    ...
    def format_error_context(error: ErrorContext) -> str:
        ...
        lines = []
        lines.append(f"🔍 ERROR DEBUG INFORMATION")
        lines.append(f"{'='*50}")
            ...
            for var_name, var_value in error.local_variables.items():
                try:
                    ...
                    if isinstance(var_value, (str, int, float, bool, type(None))):
                        ...
                        formatted_value = str(var_value)[:200] + "..." if len(str(var_value)) > 200 else str(var_value)
                    else:
...
class DebugAnalyzer:
    ...
    def analyze_error_pattern(error: ErrorContext) -> Dict[str, Any]:
        ...
        if hasattr(error, 'severity') and error.severity:
            severity_name = error.severity.name if hasattr(error.severity, 'name') else str(error.severity)
            if severity_name == 'CRITICAL':
                suggestions.append("This is a critical error that requires immediate attention")
            elif severity_name in ['HIGH', 'ERROR']:
                suggestions.append("This error may cause significant issues - review carefully")
...
class DebugReporter:
    ...
    def export_debug_report(error: ErrorContext, output_dir: str = "output") -> str:
        ...
        report_lines = []
        report_lines.append(f"GenXData Debug Report")
        ...
        # Stack trace
        report_lines.append(DebugFormatter.format_stack_trace(error, include_locals=True))
        report_lines.append("")
        ...
        # System information
        report_lines.append(f"🖥️  SYSTEM INFORMATION")

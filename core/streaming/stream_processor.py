...
def process_streaming_config(config_file, stream_config, error_handler, perf_report=False):
    """
    ...
    for batch_index, batch_size in enumerate(batches):
        logger.info(f"Processing batch {batch_index + 1}/{len(batches)} (size: {batch_size})")
        ...
        try:
            ...
            logger.debug(f"Batch {batch_index} processed successfully. Generated {len(df)} rows")

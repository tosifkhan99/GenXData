...
def process_config(config, perf_report):
    ...
    if rows < SETTINGS.MINIMUM_ROWS_ALLOWED:
        logger.warning(
            f"Requested rows ({rows}) below minimum allowed ({SETTINGS.MINIMUM_ROWS_ALLOWED}). Using minimum."
        )
                ...
                try:
                    with measure_time(
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Column {col_name} - Strategy created successfully: {strategy.__class__.__name__}"
                            )

                            df = strategy_factory.execute_strategy(strategy)
                            logger.debug(
                                f"Column {col_name} - Strategy executed successfully. Sample data: {df[col_name].head(3).tolist() if col_name in df.columns else 'N/A'}"
                            )
    ...
    logger.info(
        f"Configuration processing completed. Generated {len(df)} rows with {len(df.columns)} columns."
    )
    return df
